...
app.get('/health', (req, res) => {
  res.json({ 
    status: 'ok', 
    message: 'Fix backend server running',
...
app.get('/api/health', (req, res) => {
  res.json({ 
    status: 'ok', 
    message: 'Fix backend API running',
    timestamp: new Date().toISOString()
  });
});
...
// Update order items and details - THE MAIN FIX
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 Fix backend server running on port ${PORT}`);
